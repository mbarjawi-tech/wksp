...
**`task.agentsMd`** — the task's `AGENTS.md` content, the canonical instruction file. On import it's written to `tasks/<id>/AGENTS.md` (with a one-line `@AGENTS.md` include generated as `CLAUDE.md`). Absent in bundles created before AGENTS.md became canonical.
