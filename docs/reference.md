...
`delete`, `archive`, `finish`, `rename`, the archived-task delete, and `repo <id> <repo> share|exclude` (which remove a worktree the same way) all refuse to run while your shell is inside the task folder or one of its worktrees: `Cannot tear down <id> — your shell is inside <path>`. wksp is a child of that shell and inherits its directory, and on Windows an open directory can be neither renamed nor deleted — `cd` out (the message names the project folder) and re-run. Similar-looking siblings are safe: a shell in `tasks/foo-bar` does not block `tasks/foo`. Each worktree is then checked for removability *before* anything is removed — renamed aside and straight back, since a folder the OS lets you rename is one it will let you delete — so a folder held open by an editor, a dev server or a second shell stops the run with everything still intact (`"<repo>" is locked` … `Nothing was touched`) rather than after `git worktree remove` has already deleted its contents. `wksp repo remove` (which tears down a repo's orphaned worktrees across every task) and the project-wide `wksp delete` carry the same two checks, including for the task *folder* itself — a shell sitting in `tasks/<id>/` rather than in one of its worktrees.

If the process dies **between** that rename-away and rename-back — a crash, a kill, a power loss — the worktree is left renamed aside as a sibling of `worktrees/`, under a name that encodes which worktree it came from. It is never invisible: **every** command that looks at a task's worktrees reports it. What differs is who moves it, and that split is deliberate.

- **Commands that write to the task put it back** — `delete`, `archive`, `finish`, `rename`, `repo remove` and the project-wide `delete` were going to move or delete that worktree anyway, and `start` / `create` / `resume` exist to make the task ready to work in (they create worktrees and write `task.json` and the `.code-workspace`), so all of them recover it first and say so (`⚠ Put "<repo>" back in <id> …`), then carry on normally. Only if it truly can't be moved back does the run refuse — naming exactly what to move where, and never leading with a command that discards the task (`"nothing was touched"` is also dropped from the locked-worktree message in this one case, since the rename-away did happen). The project-wide `delete` recovers per task *after* its confirmation, so cancelling it leaves every folder exactly where it was.
- **Read commands report it and leave it alone** — `status`, `list`, `brief` and `export` name it, name the move that fixes it (`⚠ "<repo>" in <id> was left renamed aside by an interrupted run …` / `Put it back by hand: … → …`), and tell you which commands do it for you — but they never rename anything: looking at a project should not change it, and a read that renamed could land in the middle of another run's removability probe and make that run wrongly report the worktree as locked. `status` shows it as `(renamed aside)` rather than counting it as a live-but-corrupt worktree, and `list` counts it separately from the real ones.

If a worktree can't be read at all — no `.git` file, usually the wreckage of an interrupted teardown — the run does what it can and then names every step that did **not** happen for that repo (worktree removal, branch deletion, and `finish`'s fast-forward), with the `git` commands that finish the job, and exits non-zero instead of printing success. `repo <id> <repo> share` and `exclude` refuse outright in that state instead: they can't remove the folder, so recording the new mode in `task.json` would claim something the disk doesn't have — `exclude` prints the same `git -C … worktree prune` plus folder-removal pair to clear it, since tearing the whole task down is a heavy answer to excluding one repo. An `archive` that removed the worktrees but could not move the folder leaves its manifest behind on purpose: re-running `archive` recognises the half-state, says `Recovering an interrupted archive of <id>`, and re-uses that record instead of writing a poorer one.
