...
- After the merge lands, suggest \`wksp task finish ${taskId}\` — it verifies the branches are
  merged, archives the task, deletes the local branches, and fast-forwards each base repo. It has
  to run from **outside** this folder: teardown refuses while a shell sits in the task or one of
  its worktrees, because the folder it must delete would stay locked.
