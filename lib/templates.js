...
// How the hub hands work to a task without a human driving the prompts, plus the
// rule for which information lives where. Ships in the project template because
// it is the thing an AI at the project root most needs to know: planning happens
// here, code changes happen in tasks, and it can set a task up itself.
const DELEGATION_SECTION = `## Delegating work to a task (from here, headless)
...
1. \`wksp task create <id> --goal "<one line>" --branch <branch> --json\` — sets the task
   ...
   (paths, per-repo branches, where to log) as JSON on stdout.
...
4. \`wksp task brief <id>\` re-prints that context any time; \`wksp task finish <id> --yes\`
   closes the task out once its PR is merged.

Prefer a focused session in the task itself? \`wksp start <id>\`.
...
${DELEGATION_SECTION}## Cross-cutting conventions
...
module.exports = {
  ...
  WORK_LOG_SECTION, ROOT_WORK_LOG_SECTION, ROOT_PLANNING_SECTION, DELEGATION_SECTION,
};
