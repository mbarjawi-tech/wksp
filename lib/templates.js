...
// stay byte-identical. The first heading doubles as the idempotency marker.
...
⚠ \`gh stack submit\` restacks the branches itself and **rewrites SHAs**. After it runs — or
after amending anything mid-stack — re-read each branch's real history before any manual
rebase. Never trust a base SHA you remembered from before a submit.
...
- **\`gh pr merge\` is refused for stack members** — use \`gh stack merge\`. (\`gh pr merge <n>
  --repo <owner>/<repo>\` stays right for a solo PR, and \`--repo\` is what keeps gh off the
  local checkout.)
...
- **A PR that is part of a stack** is not yours to merge: \`gh pr merge\` is refused for stack
  members, and the whole stack lands together with \`gh stack merge\`, driven by whoever is
  orchestrating it. Say the branch is ready and stop there.
