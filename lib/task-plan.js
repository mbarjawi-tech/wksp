...
function planRepos({ allRepos, pending, taskId, usedNames = new Set(), opts }) {
  ...
  for (const [name, flag] of referenced) {
    ...
    // `explicit` marks a caller whose whole purpose is this one repo
    // (`wksp task repo <id> <repo> worktree`), where giving a repos.txt --shared
    // repo a task-local worktree is the documented move rather than a mistake.
    if (repo.shared && !opts.explicit && flag !== '--exclude' && flag !== '--shared') {
      errors.push({
