...
function readSet(taskDir, file) {
  const f = path.join(taskDir, file);
  if (!fs.existsSync(f)) return new Set();
  return new Set(fs.readFileSync(f, 'utf8').split('\n').map(l => l.trim()).filter(Boolean));
}

function writeSet(taskDir, file, set) {
  const f = path.join(taskDir, file);
  if (set.size === 0) { if (fs.existsSync(f)) fs.unlinkSync(f); return; }
...
function captureState(projectDir, taskId, projectName) {
  ...
  const sharedSet   = readSet(taskDir, TASK_SHARED_FILE);
  const excludedSet = readSet(taskDir, TASK_EXCLUDED_FILE);
  const wts = discoverWorktrees(taskDir);
  const wtByBase = new Map();
  for (const wt of wts) if (wt.baseRepo) wtByBase.set(normalizePath(wt.baseRepo), wt);
  ...
  for (const repo of allRepos) {
    ...
    if (excludedSet.has(repo.normalized)) {
      entries.push({ name, baseRepo: repo.normalized, status: 'excluded' });
      continue;
    }
    if (repo.shared || sharedSet.has(repo.normalized)) {
      entries.push({ name, baseRepo: repo.normalized, status: 'shared' });
      continue;
    }
    const wt = wtByBase.get(repo.normalized);
    if (!wt || wt.corrupted) {
      entries.push({
...
function archiveTask(projectDir, projectName, taskId, capturedEntries, opts = {}) {
  ...
  for (const entry of capturedEntries) {
    ...
    const wt = wtByBase.get(entry.baseRepo);
...
function buildPlan(manifest, currentRepos, overrides = {}) {
  ...
  // Drift — repos in repos.txt that weren't in the manifest
  const manifestBaseRepos = new Set(manifest.repos.map(r => r.baseRepo));
  for (const repo of currentRepos) {
    if (manifestBaseRepos.has(repo.normalized)) continue;
    const name = path.basename(repo.normalized);
    let action;
    if (skipSet.has(name))           action = { kind: 'skip' };
    else if (sharedSet.has(name))    action = { kind: 'shared' };
    else if (branchMap.has(name))    action = { kind: 'worktree', branch: branchMap.get(name) };
...
function applyPlan(projectDir, taskId, items) {
  ...
  const sharedSet   = readSet(liveDir, TASK_SHARED_FILE);
  const excludedSet = readSet(liveDir, TASK_EXCLUDED_FILE);
  ...
  for (const it of items) {
    const a = it.action;
    const baseName = path.basename(it.baseRepo);
    const wtDir = path.join(liveDir, WORKTREES_DIR, (it.entry && it.entry.folderName) || baseName);

    // Strip from both task-level sets first — we'll re-add based on action below.
    sharedSet.delete(it.baseRepo);
    excludedSet.delete(it.baseRepo);

    try {
      if (a.kind === 'shared') {
        sharedSet.add(it.baseRepo);
        successes.push(it.name);
      } else if (a.kind === 'excluded' || a.kind === 'skip') {
        excludedSet.add(it.baseRepo);
        successes.push(it.name);
  ...
  writeSet(liveDir, TASK_SHARED_FILE, sharedSet);
  writeSet(liveDir, TASK_EXCLUDED_FILE, excludedSet);
