...
const { ensureCwdOutside } = require('./teardown-guard');
...
// List the ids of archived tasks (directory names under archived-tasks/).
function listArchivedTaskIds(projectDir) {
  const root = path.join(projectDir, ARCHIVED_DIR);
...
function captureState(projectDir, taskId, projectName) {
  ...
  for (const repo of allRepos) {
    ...
    if (!wt || wt.corrupted) {
      // The worktree can't be read, so `branch` stays null and every step that keys
      // off it is skipped downstream. Recover the branch name from the base repo's
      // worktree registry anyway, into a SEPARATE field: it is what lets the run say
      // which branch was not deleted (PLANNING #20), and keeping it out of `branch`
      // keeps it out of the code paths that would act on it — nothing here verified
      // that branch was merged.
      const wtDir = wt ? wt.worktreeDir : path.join(taskDir, WORKTREES_DIR, repo.folderName);
      const reg   = git.findWorktreeEntry(repo.normalized, wtDir);
      entries.push({
        ...
        recoveredBranch: reg ? reg.branch : null,
...
// Returns { manifest, targetDir, degraded } — `degraded` lists repos whose worktree
// could not be read, so the steps this function documents (remove the worktree,
// delete its branch) did NOT run for them. The caller must report those; silence is
// what made a failed teardown look like a clean archive (PLANNING #20).
function archiveTask(projectDir, projectName, taskId, capturedEntries, opts = {}) {
  ...
  // Remove worktrees and optionally delete branches. `recover: true` — this function IS
  // the destructive step, so it puts a stranded probe back rather than leaving it for the
  // bulk folder move; handleArchive has normally already done so and refused if it could
  // not, but archiveTask must not depend on being called through it.
  const wts = discoverWorktrees(taskDir, { recover: true });

  // The other half of not depending on the caller: when that recovery FAILS, the probe is
  // still a valid git worktree sitting beside worktrees/, and falling through would record
  // it as `degraded` and then let the rename below carry the whole folder into
  // archived-tasks/ with the base repo's registration orphaned and the branch's identity
  // gone. handleArchive refuses first, with the same detail; this covers every other caller.
  const strandedProbes = wts.filter(w => w.strandedProbe);
  if (strandedProbes.length) {
    throw new Error(
      `${strandedProbes.length} worktree probe(s) in ${taskId} were left renamed aside by an interrupted\n` +
      `         run and could not be moved back:\n` +
      strandedProbes.map(w => `           ${w.strandedPath}\n             →  ${w.worktreeDir}`).join('\n') + '\n' +
      `         Move them back by hand, then re-run — archiving now would carry the folder into\n` +
      `         ${ARCHIVED_DIR}/ with its git registration orphaned.`);
  }
  ...
  for (const entry of capturedEntries) {
    ...
    if (!wt || wt.corrupted) {
      // An unreadable worktree can't be removed by git, and with `branch` null the
      // branch-deletion loop below skips it too — two documented steps that don't
      // happen. Record it instead of continuing quietly. A repo that simply never had
      // a worktree AND has no registration left is not degraded: there is nothing
      // that was supposed to happen for it.
      if (wt || entry.recoveredBranch) {
        degraded.push({
          name:     entry.name,
          baseRepo: entry.baseRepo,
          branch:   entry.recoveredBranch || null,
          reason:   entry.note || 'worktree unreadable',
        });
      }
      continue;
    }
  ...
  if (opts.deleteBranches) {
    for (const entry of capturedEntries) {
      ...
      const r = git.deleteBranch(entry.baseRepo, entry.branch, !!(opts.force || opts.forceDeleteBranches));
      // `git branch -D` also fails when an earlier interrupted run already deleted the
      // branch — that is "gone", not "kept", so check before recording it.
      entry.branchKeptInBaseRepo = !r.ok && git.branchExistsLocally(entry.baseRepo, entry.branch);
    }
  }
  ...
  // Last chance to get out of the way: our own cwd inside the folder makes rename fail
  // with EPERM on Windows for the same reason it makes rmSync fail with EBUSY in
  // lib/commands/delete.js. The teardown preflight already refused a cwd inside the
  // task — this covers a cwd it judged outside that the OS still counts as inside.
  ensureCwdOutside(taskDir, projectDir);

  try {
    fs.renameSync(taskDir, targetDir);
  } catch (e) {
    // The manifest above lives INSIDE the folder that just failed to move, which is
    // exactly the half-state a locked folder left behind: a live task carrying a
    // stale archived.json. It is kept deliberately — handleArchive recognises it and
    // re-uses it on the next run, because re-capturing now (worktrees already gone)
    // would write a manifest with no branch and no tip sha.
    throw new Error(
      `could not move tasks/${taskId}/ into ${ARCHIVED_DIR}/ (${e.code || e.message}).\n` +
      `         Its worktrees were already removed and its manifest written, so the task is\n` +
      `         half-archived. Close whatever holds the folder open (a shell, editor or dev\n` +
      `         server inside it) and re-run — the next run finishes the move.`);
  }

  return { manifest, targetDir, degraded };
}
...
function classifyRepo(entry) {
  ...
  // FOLLOW-UP: same ancestry-only assumption as handleFinish once had — a squash-/
  // rebase-merged tip won't be an ancestor here either, so it falls through to
  // 'dangling'. Left as-is to keep the merge-check-gh PR focused; a later change
  // could consult lib/forge.prMergeState here too (see PLANNING.md #14).
...
function resolveCheckoutConflict(baseRepo, action) {
  ...
  // CANONICAL: `checkedOutAt` comes back from `git worktree list` in git's long
  // on-disk form, `baseRepo` is the registry's spelling. The old `path.resolve(a) ===
  // path.resolve(b)` was wrong twice over — it compared two spellings of one
  // directory as strings, and did it case-sensitively on a case-insensitive
  // filesystem. Both made unarchive mis-read "the branch is checked out in the base
  // repo itself" as "checked out in some other worktree" and skip the repo.
  if (samePathCanonical(checkedOutAt, baseRepo)) {
    return { kind: 'shared' };
  }
...
module.exports = {
  ...
  archivedTaskDir, liveTaskDir, listArchivedTaskIds,
