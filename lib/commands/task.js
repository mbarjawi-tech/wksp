...
function reportPartialSetup(taskId, repoName, detail) {
  ...
  console.error('  Fix the cause above, then finish the task with:');
  console.error(`      wksp start ${taskId}`);
  console.error('  It resumes where this stopped, asking only about repos that still have no answer.');
  console.error(`  To discard it instead:  wksp task delete ${taskId}\n`);
}
