...
async function handleToExclude(projectDir, taskId, repoArg) {
  if (!repoArg) { console.error('  Usage: wksp task <id> --to-exclude <repo-name>'); process.exit(1); }
...
async function handleRename(projectDir, projectName, oldId, newId) {
  if (!newId) { console.error('  Usage: wksp task <id> --rename <new-id>'); process.exit(1); }
...
async function run(args) {
  ...
  if (!taskId || flags.has('--help') || args.includes('-h') || taskId === '--help') {
    console.log(`
  wksp task <id>                   Open or create a task workspace

  Options:
    --del                          Tear down worktrees and delete the task folder
    --rename <new-id>              Rename the task (no Claude launch)
    --archive                      Archive the task
    --unarchive                    Restore an archived task
    --to-shared <repo>             Use shared path for <repo> in this task only
    --to-worktree <repo>           Create a worktree for a repo that was shared
    --to-exclude <repo>            Exclude <repo> from this task
...
`);
  ...
  if (flags.has('--rename'))      { await handleRename(projectDir, projectName, taskId, posArgs[1]); return; }
  ...
  if (flags.has('--to-exclude'))  { await handleToExclude(projectDir, taskId, posArgs[1]); return; }
