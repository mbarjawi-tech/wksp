...
// A setup run that stopped part-way. Every decision made before this point is already
// on disk (worktrees by git, shared/excluded by task.json), so the task is not broken —
// it is unfinished, and re-running finishes it. Say that, rather than leaving a raw git
// error as the last word and a task the obvious retry (`wksp task create`) refuses.
// `reRun` matters: the default only fits a SETUP run. `wksp start` asks about repos
// that still have no answer, so pointing a failed `task repo … worktree` at it would
// name a command that does nothing — that repo still holds the disposition this fix
// deliberately preserved. Callers that aren't setting the whole task up pass their own
// command, and suppress the discard line, since the task itself is not in question.
function reportPartialSetup(taskId, repoName, detail, opts = {}) {
  if (repoName) console.error(`\n  ✗  Could not set up "${repoName}" in ${taskId}`);
  if (detail)   console.error(`     ${detail}`);
  // The default suits a setup run, which may genuinely have decided several repos
  // before stopping. A single-repo verb decided nothing at all, so it says so instead.
  console.error(`\n  ${opts.headline || 'Everything decided before this point has been saved, and nothing was undone.'}`);
  console.error('  Fix the cause above, then re-run:');
  console.error(`      ${opts.reRun || `wksp start ${taskId}`}`);
  console.error(`  ${opts.reRunNote || 'It resumes where this stopped, asking only about repos that still have no answer.'}`);
  if (opts.discard === false) console.error('');
  else console.error(`  To discard the task instead:  wksp task delete ${taskId}\n`);
}
...
async function handleToShared(projectDir, taskId, repoArg, rawOpts = {}) {
  ...
  if (!wt) {
    // No worktree is not automatically an error: a repo can be registered in repos.txt
    // and have no disposition in this task at all (a setup run that stopped part-way),
    // and "make it shared" is exactly how you finish that off. `exclude` already worked
    // this way; requiring a worktree here made the one state that needed repairing the
    // one state this verb refused to touch. There is nothing to remove — just record it.
    const registered = readRepos(projectDir).find(r => r.folderName === repoArg || r.raw === repoArg);
    if (registered) {
      const { taskSharedSet: s, taskExcludedSet: x } = readTaskSets(taskDir);
      if (s.has(registered.folderName)) {
        console.log(`  "${registered.folderName}" already uses the shared repo path in this task.`);
        return;
      }
      x.delete(registered.folderName);
      s.add(registered.folderName);
      writeTaskSets(taskDir, s, x);
      console.log(`  ✓  ${registered.folderName} now uses the shared repo path`);
      console.log(`     (${registered.normalized})`);
      return;
    }
    console.error(`  Error: no worktree or registered repo matching "${repoArg}" in task ${taskId}`);
    if (wts.length) console.error(`  Available worktrees: ${wts.map(w => w.folderName).join(', ')}`);
    process.exit(1);
  }
  if (wt.corrupted || !wt.baseRepo) {
...
async function handleToWorktree(projectDir, taskId, repoArg, rawOpts = {}) {
  ...
  const { taskSharedSet, taskExcludedSet } = readTaskSets(taskDir);
  // Read, don't strip. The repo keeps whatever this task already recorded until the
  // worktree genuinely exists: erasing it up front meant every failure below — a bad
  // branch, a repo that isn't on disk, a fetch that didn't land — left the repo in no
  // state at all, which is precisely the damage this command is most often run to
  // repair. It also erased the disposition on the "already has a worktree" early
  // return, which changes nothing and should record nothing.
  const wasShared   = taskSharedSet.has(repo.folderName);
  const wasExcluded = taskExcludedSet.has(repo.folderName);
  ...
  if (existing) {
    ...
    if (existing.strandedProbe) {
      ...
      // A healthy worktree and a leftover shared/excluded entry contradict each other,
      // and the disk is the truth. Reconcile here — this verb declining to record is
      // right when it did no work, but leaving a provably-wrong record in place would
      // make `status` report a mode the task does not have, with nothing left to fix it.
      //
      // Confirm the worktree is actually THIS repo's before acting on it: worktrees are
      // matched by folder name, and two registered repos can share a basename
      // (C:/x/api and C:/y/api). Clearing repo1's correct record on the strength of
      // repo2's worktree would be the same class of wrong answer this reconciliation
      // exists to remove. It also rules out a worktree whose gitdir no longer resolves.
      const sameRepo = existing.baseRepo && samePathCanonical(existing.baseRepo, repo.normalized);
      if (sameRepo && (wasShared || wasExcluded)) {
        taskSharedSet.delete(repo.folderName);
        taskExcludedSet.delete(repo.folderName);
        writeTaskSets(taskDir, taskSharedSet, taskExcludedSet);
        console.log(`     (cleared a stale "${wasShared ? 'shared' : 'excluded'}" record that contradicted it)`);
      }
    }
    return;
  }
  ...
  if (branchGiven) {
    ...
    applyRepoPlan(items, {
      ...
      fail: msg => {
        ...
        reportPartialSetup(taskId, null, null, {
          headline:  'Nothing was changed — the task is exactly as it was.',
          reRun:     `wksp task repo ${taskId} ${repoArg} worktree --branch <branch>`,
          reRunNote: `Only ${repoArg} is still without a worktree.`,
          discard:   false,
        });
        process.exit(1);
      },
      stdio: 'inherit',
      persist: () => writeTaskSets(taskDir, taskSharedSet, taskExcludedSet),
    });
    return;
  }
  ...
  let result;
  try {
    result = await createWorktree(repo, taskDir, usedNames);
  } catch (e) {
    // Nothing has been written, so the repo still holds whatever it held before —
    // which is exactly why `wksp start` would be the wrong thing to suggest here.
    close();
    reportPartialSetup(taskId, repo.folderName, String(e.message || e).split('\n')[0], {
      headline:  'Nothing was changed — the task is exactly as it was.',
      reRun:     `wksp task repo ${taskId} ${repoArg} worktree`,
      reRunNote: `Only ${repoArg} is still without a worktree.`,
      discard:   false,
    });
    process.exit(1);
  ...
  taskSharedSet.delete(repo.folderName);
  taskExcludedSet.delete(repo.folderName);
  if (result.kind === 'shared')   { taskSharedSet.add(repo.folderName);   console.log(`  Kept ${repoArg} as shared.`); }
  if (result.kind === 'excluded') { taskExcludedSet.add(repo.folderName); console.log(`  Kept ${repoArg} excluded.`); }
  writeTaskSets(taskDir, taskSharedSet, taskExcludedSet);
}
...
// ask a question. Mutates the two disposition sets.
// Shared by headless create/resume and `wksp task repo <id> <repo> worktree`.
//
// `persist` is called after EVERY decision rather than once at the end. A worktree is
// durable the moment git creates it, but `shared` and `excluded` live only in
// task.json — so a single write after the loop meant any failure part-way discarded
// every answer already given, leaving those repos in no state at all (`mode: missing`,
// silently absent from the launch context). Writing as we go costs one small JSON
// write per repo and makes a partial run resumable instead of lossy.
function applyRepoPlan(items, { taskDir, taskSharedSet, taskExcludedSet, usedNames, fail, stdio, persist = () => {} }) {
  const created = new Set();
  for (const item of items) {
    if (item.mode === 'shared') {
      taskExcludedSet.delete(item.name); taskSharedSet.add(item.name);
      persist(); console.log(`  ✓  ${item.name} → shared`); continue;
    }
    if (item.mode === 'excluded') {
      taskSharedSet.delete(item.name); taskExcludedSet.add(item.name);
      persist(); console.log(`  ✓  ${item.name} → excluded from this task`); continue;
    }

    const worktreeDir = path.join(taskDir, WORKTREES_DIR, item.name);
    console.log(`\n  Creating worktree for ${item.name} on "${item.branch}" ...`);
    // A repo being RE-dispositioned to a worktree keeps its existing shared/excluded
    // entry until the worktree actually exists. Stripping it first and persisting on
    // the failure path would destroy a still-valid disposition that nothing on disk
    // had changed — leaving the repo in no state at all, which is the very defect
    // this persistence exists to prevent (and would delete task.json outright when
    // it held only that one entry).
    try {
      git.addWorktree(item.repo.normalized, worktreeDir, item.branch, item.baseBranch, stdio);
    } catch (e) {
      persist();  // sets are untouched for THIS repo; earlier decisions are kept
      fail(`could not create the worktree for ${item.name} on "${item.branch}": ${String(e.message || e).split('\n')[0]}`);
      // Every `fail` exits, but the lines below now WRITE disposition state — so a
      // caller that ever returned would record a worktree that does not exist.
      continue;
    }
...
async function handleOpen(projectDir, projectName, taskId, mode, rawOpts = {}) {
  ...
  // Persist after every decision, not once at the end — see applyRepoPlan. Anything
  // that stops this command part-way (a worktree git won't create, Ctrl-C at a prompt,
  // stdin closing) must not cost the answers already given.
  const persist = () => writeTaskSets(taskDir, taskSharedSet, taskExcludedSet);

  for (const n of optionalNames) taskExcludedSet.add(n);
  if (optionalNames.size) persist();

  // Same contract as the interactive path: a worktree that can't be created ends the
  // run, but says what survived and which command finishes the job.
  const failSetup = (message, extra) => {
    if (o.json) failJson(message, { ...(extra || {}), partial: true, recoverWith: `wksp start ${taskId}` });
    console.error(`\n  Error: ${message}`);
    reportPartialSetup(taskId, null, null);
    process.exit(1);
  };
  ...
  const createdWorktrees = applyRepoPlan(items, {
    taskDir, taskSharedSet, taskExcludedSet, usedNames, fail: failSetup, stdio: childStdio(o.json), persist,
  });
  ...
  if (toPrompt.length) {
    ...
    for (const repo of toPrompt) {
      ...
      if (!fs.existsSync(repo.normalized)) {
        // Deliberately left undecided rather than auto-excluded: an excluded repo is
        // never asked about again, so a path you go on to fix would stay silently out
        // of the task. Undecided means the next run asks — and `status` now says so.
        console.warn(`  ⚠  Repo not found on disk: ${repo.normalized}`);
        console.warn(`     ${repo.folderName} is not part of this task — fix the path in repos.txt and re-run.`);
        continue;
      }
      let result;
      try {
        ...
        // Everything decided so far is already on disk; say what survived and how to
        // carry on, instead of dying with a bare `Fatal:` and a raw git command.
        close();
        reportPartialSetup(taskId, repo.folderName, String(e.message || e).split('\n')[0]);
        process.exit(1);
      }
