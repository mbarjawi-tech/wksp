...
function applyRepoPlan(items, { taskDir, taskSharedSet, taskExcludedSet, usedNames, fail, stdio, persist = () => {} }) {
  ...
  for (const item of items) {
    ...
    if (item.mode === 'shared')   { taskSharedSet.add(item.name);   persist(); console.log(`  ✓  ${item.name} → shared`); continue; }
    if (item.mode === 'excluded') { taskExcludedSet.add(item.name); persist(); console.log(`  ✓  ${item.name} → excluded from this task`); continue; }
    ...
    try {
      ...
      persist();  // keep every decision made before this one
