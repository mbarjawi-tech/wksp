...
//
// Shared by every command here that removes a worktree — delete / archive / finish,
// the archived-task delete, and `repo <id> <repo> share|exclude` — so the wording and
// the order of operations stay in one place. (`wksp repo remove` and the project-wide
// `wksp delete` reuse the pure isCwdInside/probeRemovable checks directly instead,
// with their own wording: "Cannot tear down <taskId>" doesn't fit either — they can
// touch many tasks, or the whole project, in one run.)
function refuseIfCwdInside(dir, taskId, projectDir) {
  if (!isCwdInside(dir)) return;
  console.error(`\n  Cannot tear down ${taskId} — your shell is inside ${currentCwd()}.`);
  console.error('  wksp inherits that directory from the shell that launched it, so the folder stays');
  console.error('  locked (Windows) and a half-removed worktree is the usual result.');
...
// is supplied whole rather than assembled from a verb + id.
function refuseIfLocked(wts, taskDir, taskId, reRunCmd) {
  for (const wt of wts) {
    if (wt.corrupted) continue; // nothing intact left to protect
    const probe = probeRemovable(wt.worktreeDir, taskDir);
    if (probe.ok) continue;
    console.error(`\n  Cannot tear down ${taskId} — "${wt.folderName}" is locked${probe.code ? ` (${probe.code})` : ''}.`);
    console.error('  Something still has it open (a shell, editor, dev server or file watcher).');
    if (probe.stranded) {
      // The rename-away half of the probe succeeded, so it DID touch the folder (to
      // test it) — only the rename-back failed. Saying "nothing was touched" here
      // would contradict the "move it back" instruction printed right below.
    ...
    if (probe.stranded) {
      console.error(`\n  ⚠  The lock check could not put the folder back. It is now at:\n       ${probe.stranded}`);
...
// could not put it back automatically (recoverStrandedProbes, called from
// discoverWorktrees, already tried). Continuing would let the eventual bulk
...
async function teardownTask(taskDir, taskId, opts = {}) {
  ...
  // All refusals come first: nothing below this point is undoable.
...
async function handleToShared(projectDir, taskId, repoArg, rawOpts = {}) {
  ...
  refuseIfCwdInside(wt.worktreeDir, taskId, projectDir);
  refuseIfLocked([wt], taskDir, taskId, `repo ${taskId} ${repoArg} share`);
...
async function handleToExclude(projectDir, taskId, repoArg, rawOpts = {}) {
  ...
  if (wt && !wt.corrupted) {
    ...
    refuseIfCwdInside(wt.worktreeDir, taskId, projectDir);
    refuseIfLocked([wt], taskDir, taskId, `repo ${taskId} ${repoArg} exclude`);
...
async function handleFinish(projectDir, projectName, taskId, opts) {
  ...
  const allWts = discoverWorktrees(taskDir);
