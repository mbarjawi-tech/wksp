...
async function teardownTask(taskDir, taskId, opts = {}) {
  const { auto = false, force = false, deleteBranches = false, forceBranches = false } = opts;
...
async function handleFinish(projectDir, projectName, taskId, opts) {
  ...
  for (const wt of wts) {
    ...
    if (def && wt.currentBranch === def) {
      ...
      if (!merged) {
        forgeResult = forge.prMergeState(wt.baseRepo, wt.currentBranch);
  ...
  if (unmerged.length) {
    // An OPEN PR is a definite "not merged"; the squash-merge hedge only fits the
    // inconclusive (unknown / ancestry-only) cases. Use the hedge headline unless
    // every unmerged branch is a known-open PR.
    const isOpenPr = c => !!(c.forge && c.forge.state === 'unmerged' && c.forge.pr && c.forge.pr.number);
    if (unmerged.every(isOpenPr)) {
      console.log(`\n  ⚠  The following branch(es) have an open pull request — not merged yet.`);
      console.log('     Verify the PR before continuing:');
    } else {
  ...
  if (opts.noArchive) {
    ...
    await teardownTask(taskDir2, taskId, {
      auto:           opts.yes,
      force:          opts.force,
...
async function resolveTaskId(projectDir, sub, provided, opts = {}) {
  ...
  if (provided) {
    ...
    // (handleDel deletes it); otherwise try archived partial matches.
