...
const TASK_OWN_DEPS_FILE = 'task-own-deps.txt';

function readTaskOwnDeps(taskDir) {
  const f = path.join(taskDir, TASK_OWN_DEPS_FILE);
  if (!fs.existsSync(f)) return new Set();
  return new Set(fs.readFileSync(f, 'utf8').split('\n').map(l => l.trim()).filter(Boolean));
}

function writeTaskOwnDeps(taskDir, set) {
  const f = path.join(taskDir, TASK_OWN_DEPS_FILE);
...
// Read sharedDeps from config, validating it's actually an array.
// Returns [] and warns if the value is set but not an array (e.g. user ran
// `wksp config set sharedDeps true` instead of `["node_modules"]`).
function getSharedDeps(projectDir) {
  const { sharedDeps } = config.readConfig(projectDir);
  if (sharedDeps === undefined || sharedDeps === null) return [];
  if (!Array.isArray(sharedDeps)) {
    console.warn(`  ⚠  sharedDeps in .wksp must be a JSON array (e.g. ["node_modules"]) — current value ignored.`);
    console.warn(`     To fix: wksp config set sharedDeps '["node_modules"]'`);
    return [];
  }
  return sharedDeps;
}
...
async function handleOwnDeps(projectDir, taskId, repoArg) {
  if (!repoArg) { console.error('  Usage: wksp task repo <id> <repo> own-deps'); process.exit(1); }
  ...
  const sharedDeps = getSharedDeps(projectDir);
  if (sharedDeps.length === 0) {
    console.warn('  No sharedDeps configured in .wksp — nothing to do.');
    return;
  }
  ...
  // Remove any symlinks for shared deps
  deps.removeDepLinks(wt.worktreeDir, sharedDeps);

  // Add to own-deps list
  const taskOwnDepsSet = readTaskOwnDeps(taskDir);
  taskOwnDepsSet.add(wt.folderName);
  writeTaskOwnDeps(taskDir, taskOwnDepsSet);

  console.log(`  ✓  ${wt.folderName}: switched to own deps (shared links removed).`);
  console.log(`     Run your install command inside the worktree to set up deps independently.`);
}

async function handleLinkDeps(projectDir, taskId, repoArg) {
  if (!repoArg) { console.error('  Usage: wksp task repo <id> <repo> link-deps'); process.exit(1); }
  ...
  // Check for real dep dirs that would block linking
  for (const dep of sharedDeps) {
    if (deps.hasRealDepDir(wt.worktreeDir, dep)) {
      console.error(`  Error: ${wt.folderName}/${dep} is a real directory (not a symlink).`);
      console.error(`         Remove it first, then run link-deps again.`);
      process.exit(1);
    }
  }

  const cacheDir = deps.getCacheDir(projectDir, wt.folderName);
  deps.ensureCacheDir(projectDir, wt.folderName, sharedDeps);
  deps.createDepLinks(wt.worktreeDir, cacheDir, sharedDeps);

  // Remove from own-deps list
  const taskOwnDepsSet = readTaskOwnDeps(taskDir);
  taskOwnDepsSet.delete(wt.folderName);
  writeTaskOwnDeps(taskDir, taskOwnDepsSet);

  console.log(`  ✓  ${wt.folderName}: deps linked → .wksp-cache/${wt.folderName}/`);
}
...
async function handleArchive(projectDir, projectName, taskId, opts) {
  ...
  const ownDepsRepos = [...readTaskOwnDeps(taskDir)];
  ...
  try {
    archive.archiveTask(projectDir, projectName, taskId, entries, { ...opts, ownDepsRepos });
    console.log(`\n  ✓  Archived ${taskId}.`);
...
async function handleUnarchive(projectDir, projectName, taskId, opts) {
  ...
  // Restore own-deps list and recreate shared dep links for other worktrees
  const taskDir    = path.join(projectDir, 'tasks', taskId);
  const ownDepsFromManifest = manifest.ownDepsRepos || [];
  if (ownDepsFromManifest.length > 0) {
    writeTaskOwnDeps(taskDir, new Set(ownDepsFromManifest));
  }
  const sharedDeps = getSharedDeps(projectDir);
  if (sharedDeps.length > 0) {
    const ownDepsSet = new Set(ownDepsFromManifest);
    const restoredWts = discoverWorktrees(taskDir);
    for (const wt of restoredWts) {
      if (!wt.worktreeDir || !fs.existsSync(wt.worktreeDir)) continue;
      if (ownDepsSet.has(wt.folderName)) continue;
      const cacheDir = deps.getCacheDir(projectDir, wt.folderName);
      deps.ensureCacheDir(projectDir, wt.folderName, sharedDeps);
      deps.createDepLinks(wt.worktreeDir, cacheDir, sharedDeps);
    }
...
async function handleOpen(projectDir, projectName, taskId, mode) {
  ...
  if (sharedDeps.length > 0) {
    const taskOwnDepsSet = readTaskOwnDeps(taskDir);
    let ownDepsChanged = false;
    for (const wt of finalWts) {
      if (!wt.worktreeDir || !fs.existsSync(wt.worktreeDir)) continue;
      if (taskOwnDepsSet.has(wt.folderName)) continue;
      let autoOptOut = false;
      for (const dep of sharedDeps) {
        if (deps.hasRealDepDir(wt.worktreeDir, dep)) { autoOptOut = true; break; }
      }
      if (autoOptOut) {
        taskOwnDepsSet.add(wt.folderName);
        ownDepsChanged = true;
        console.warn(`  ⚠  ${wt.folderName}: existing installed deps found — marked as own-deps (not shared).`);
        console.warn(`     To switch to shared: wksp task repo ${taskId} ${wt.folderName} link-deps`);
      } else {
        const cacheDir = deps.getCacheDir(projectDir, wt.folderName);
        deps.ensureCacheDir(projectDir, wt.folderName, sharedDeps);
        deps.createDepLinks(wt.worktreeDir, cacheDir, sharedDeps);
    ...
    if (ownDepsChanged) writeTaskOwnDeps(taskDir, taskOwnDepsSet);
  }
...
async function handleRepo(projectDir, projectName, taskId, repoArg, modeArg) {
  ...
  const VALID_MODES = ['share', 'worktree', 'exclude', 'own-deps', 'link-deps'];

  // Validate args before opening any prompt
  if (modeArg && !VALID_MODES.includes(modeArg)) {
    console.error(`  Error: unknown mode "${modeArg}". Use: share, worktree, exclude, own-deps, or link-deps`);
    process.exit(1);
  }
  ...
  if (!mode) {
    ...
    console.log('    own-deps  — remove shared dep links; use independent install');
    console.log('    link-deps — restore shared dep links from .wksp-cache/');
    const input = (await ask('\n  Mode [share / worktree / exclude / own-deps / link-deps]: ')).trim();
  ...
  if (mode === 'own-deps')  await handleOwnDeps(projectDir, taskId, repo.folderName);
  if (mode === 'link-deps') await handleLinkDeps(projectDir, taskId, repo.folderName);
}
...
async function handleLegacy(args) {
  ...
  if (!taskId) { console.error('  Usage: wksp task <subcommand> <id>'); process.exit(1); }
  ...
  if (flags.has('--del')) {
    warnDeprecated(`wksp task ${taskId} --del`, `wksp task delete ${taskId}`);
    const { opts } = parseArchiveArgs(args);
    await handleDel(projectDir, taskId, opts);
  } else if (flags.has('--rename')) {
    const newId = posArgs[1];
    warnDeprecated(`wksp task ${taskId} --rename ${newId || '<new-id>'}`, `wksp task rename ${taskId} ${newId || '<new-id>'}`);
    await handleRename(projectDir, projectName, taskId, newId);
  } else if (flags.has('--archive')) {
    warnDeprecated(`wksp task ${taskId} --archive`, `wksp task archive ${taskId}`);
    const { opts } = parseArchiveArgs(args);
    await handleArchive(projectDir, projectName, taskId, opts);
  } else if (flags.has('--unarchive')) {
    warnDeprecated(`wksp task ${taskId} --unarchive`, `wksp task unarchive ${taskId}`);
    const { opts } = parseUnarchiveArgs(args);
    await handleUnarchive(projectDir, projectName, taskId, opts);
  } else if (flags.has('--to-shared')) {
    warnDeprecated(`wksp task ${taskId} --to-shared ${posArgs[1] || '<repo>'}`, `wksp task repo ${taskId} ${posArgs[1] || '<repo>'} share`);
    await handleToShared(projectDir, taskId, posArgs[1]);
  } else if (flags.has('--to-worktree')) {
    warnDeprecated(`wksp task ${taskId} --to-worktree ${posArgs[1] || '<repo>'}`, `wksp task repo ${taskId} ${posArgs[1] || '<repo>'} worktree`);
    await handleToWorktree(projectDir, taskId, posArgs[1]);
  } else if (flags.has('--to-exclude')) {
    warnDeprecated(`wksp task ${taskId} --to-exclude ${posArgs[1] || '<repo>'}`, `wksp task repo ${taskId} ${posArgs[1] || '<repo>'} exclude`);
    await handleToExclude(projectDir, taskId, posArgs[1]);
  } else {
    const isNew = !fs.existsSync(path.join(projectDir, 'tasks', taskId));
    warnDeprecated(`wksp task ${taskId}`, isNew ? `wksp task create ${taskId}` : `wksp task resume ${taskId}`);
    await handleOpen(projectDir, projectName, taskId, null);
  }
}
...
async function run(args) {
  ...
  Subcommands:
    ...
    repo <id> [repo] [mode]        Configure how a repo participates in this task
                                   Modes: share, worktree, exclude, own-deps, link-deps
