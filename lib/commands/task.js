...
function refuseIfCwdInside(dir, taskId, projectDir) {
  ...
  console.error(`\n  Cannot tear down ${taskId} — your shell is inside ${currentCwd() || dir}.`);
...
function refuseIfLocked(wts, taskDir, taskId, verb) {
  for (const wt of wts) {
    ...
    console.error(`  Then re-run: wksp task ${verb} ${taskId}\n`);
    process.exit(1);
  }
}
...
function reportSkippedSteps(degraded, taskId, { ff = false, branches = true } = {}) {
  ...
  for (const d of degraded) {
    ...
    if (!d.baseRepo) {
      console.warn('         (base repo unknown — find it in repos.txt, then run the commands below in it)');
    }
...
async function teardownTask(taskDir, taskId, opts = {}) {
  ...
  // Both refusals come first: nothing below this point is undoable.
  refuseIfCwdInside(taskDir, taskId, projectDir);
  const wts = discoverWorktrees(taskDir);
  refuseIfLocked(wts, taskDir, taskId, verb);
...
async function handleArchive(projectDir, projectName, taskId, opts) {
  ...
  refuseIfLocked(wts, taskDir, taskId, 'archive');
