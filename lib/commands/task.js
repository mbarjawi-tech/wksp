...
const { launch, findLastSession, sessionDirsFor, migrateSessionDir } = require('../claude');
...
async function migrateRenamedSessions(oldTaskDir, newTaskDir, opts) {
  const { from, to, sessionCount, targetExists } = sessionDirsFor(oldTaskDir, newTaskDir);
  ...
  if (res.moved || res.merged) console.log(`  ✓  Migrated session history → ${to}`);
...
function lastActivity(taskDir) {
  const session = findLastSession(taskDir);
...
const PICKER_VERBS = { resume: 'Resume', delete: 'Delete', archive: 'Archive' };
...
const PICKER_SUBS = new Set(['resume', 'delete', 'archive']);
...
async function run(rawArgs) {
  ...
  resume / delete / archive: omit the id to pick from a list, or pass part of a
