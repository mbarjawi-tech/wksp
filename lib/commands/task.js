...
async function handleHub(projectDir, projectName, mode) {
  ...
  if (!exists) {
    ...
    console.log(`\n  Created the ${HUB_TASK_ID} — the project's planning task (no worktree).`);
    console.log(`  Track the feature backlog and cross-cutting design in tasks/${HUB_TASK_ID}/CLAUDE.md.`);
  } else {
    console.log(`\n  Resuming the ${HUB_TASK_ID}: ${HUB_TASK_ID}`);
  }
