...
function writeWorkspaceFile(taskDir, projectName, taskId, allRepos, taskSharedSet, finalBaseMap) {
  const folders = [];
...
async function createWorktree(repo, taskDir, usedNames) {
  ...
  let branch;
  while (true) {
    const input = await ask(`\n  Branch for ${repoName} [${repoBranch}, s=shared]: `);
    if (input === 's') {
      console.log(`  ✓  ${repoName} → shared`);
      return null; // caller adds to taskSharedSet
    }
    ...
    if (conflict) {
      ...
      console.log('  Enter a different branch, s=shared, or use --to-shared later.\n');
      continue;
    }
    break;
  }
...
async function handleToWorktree(projectDir, taskId, repoArg) {
  ...
  close();
  if (result === null) console.log(`  Kept ${repoArg} as shared.`);
}
...
async function run(args) {
  ...
  if (newRepos.length) {
    ...
    for (const repo of newRepos) {
      ...
      const result = await createWorktree(repo, taskDir, usedNames);
      if (result === null) taskSharedSet.add(repo.normalized);
    }
    close();
    writeTaskShared(taskDir, taskSharedSet);
  }
  ...
  writeWorkspaceFile(taskDir, projectName, taskId, allRepos, taskSharedSet, finalBaseMap);
