...
async function handleOpen(projectDir, projectName, taskId, mode, rawOpts = {}) {
  ...
  // Apply the validated plan. Nothing here can ask a question.
  const createdWorktrees = new Set();
  const stdio = childStdio(o.json);
  for (const item of items) {
    ...
    createdWorktrees.add(item.name);
...
const OPEN_SUBS = new Set(['create', 'resume', 'brief']);
