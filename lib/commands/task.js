...
function writeWorkspaceFile(taskDir, projectName, taskId, allRepos, taskSharedSet, taskExcludedSet, finalBaseMap) {
  ...
  for (const repo of allRepos) {
    if (taskExcludedSet.has(repo.normalized)) continue;
...
async function handleToWorktree(projectDir, taskId, repoArg) {
  ...
  const wasShared   = taskSharedSet.delete(repo.normalized);
  const wasExcluded = taskExcludedSet.delete(repo.normalized);
  writeTaskShared(taskDir, taskSharedSet);
  ...
  if (result.kind === 'shared') {
    taskSharedSet.add(repo.normalized);
    writeTaskShared(taskDir, taskSharedSet);
    console.log(`  Kept ${repoArg} as shared.`);
  } else if (result.kind === 'excluded') {
    taskExcludedSet.add(repo.normalized);
    writeTaskExcluded(taskDir, taskExcludedSet);
...
async function run(args) {
  ...
  const newRepos = allRepos.filter(r =>
    !r.shared &&
    !taskSharedSet.has(r.normalized) &&
    !taskExcludedSet.has(r.normalized) &&
    !existingBaseMap.has(r.normalized)
  );

  if (newRepos.length) {
    open();
    for (const repo of newRepos) {
      if (!isNew) console.log(`\n  New repo in repos.txt: ${path.basename(repo.normalized)} — pick branch, share, or exclude.`);
      if (!fs.existsSync(repo.normalized)) {
        console.warn(`  ⚠  Repo not found on disk: ${repo.normalized} — skipping`); continue;
      }
      const result = await createWorktree(repo, taskDir, usedNames);
      if (result.kind === 'shared')   taskSharedSet.add(repo.normalized);
      if (result.kind === 'excluded') taskExcludedSet.add(repo.normalized);
    }
    close();
    writeTaskShared(taskDir, taskSharedSet);
    writeTaskExcluded(taskDir, taskExcludedSet);
  ...
  const repoInfos = allRepos.map(repo => {
    const name = path.basename(repo.normalized);
    if (taskExcludedSet.has(repo.normalized)) {
