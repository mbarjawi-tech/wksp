...
const { taskAgentsMd, writeInstructionFiles } = require('../templates');
...
// Tear down a live task: remove its worktrees (force-prompting on uncommitted
// changes), optionally delete the resulting local branches, then delete the task
// folder. Shared by `delete` and `finish --no-archive`. Assumes any confirm-to-
// proceed prompt already happened; the prompt lifecycle (open/close) is managed
// here. `askBranchDeletion` lets finish suppress the interactive branch prompt.
async function teardownTask(taskDir, taskId, { askBranchDeletion = true } = {}) {
  ...
  if (askBranchDeletion && removed.length) {
...
async function handleDel(projectDir, taskId, opts = {}) {
  ...
  await teardownTask(taskDir, taskId);
}
...
function parseFinishArgs(args) {
  const opts = { keepBranches: false, force: false, reason: null, yes: false, noArchive: false };
  let taskId = null;
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a === '--keep-branches') { opts.keepBranches = true; continue; }
    if (a === '--no-archive' || a === '--delete') { opts.noArchive = true; continue; }
    if (a === '--force')         { opts.force = true; continue; }
...
async function handleFinish(projectDir, projectName, taskId, opts) {
  ...
  // ff-update helper: safe-update each base repo's default branch — fast-forward
  // only, and only when the repo is clean and already on that branch.
  ...
  // --no-archive: delete the task outright instead of archiving. Still verify
  // merged (above) and fast-forward the base repos, but keep no archive copy.
  if (opts.noArchive) {
    console.log(`\n  About to finish ${taskId} with NO archive kept:`);
    if (wts.length) console.log(`    · Remove worktrees: ${wts.map(w => w.folderName).join(', ')}`);
    console.log(`    · Delete local branches in base repos`);
    console.log(`    · Delete folder: tasks/${taskId}/ and all contents`);
    console.log(`    · This is irreversible — no archive is created.\n`);
    if (!opts.yes) {
      open();
      const yes = await confirm(`  Delete ${taskId} permanently (no archive)?`);
      close();
      if (!yes) { console.log('  Cancelled.'); return; }
    }

    // ff-update the base repos BEFORE tearing down worktrees — deleting the task's
    // worktrees doesn't touch the base repo's own default-branch checkout, but do
    // it first so the update runs while everything is still in place.
    ffUpdateBaseRepos();

    // Reuse delete's teardown; suppress its branch-deletion prompt since finish's
    // contract is to delete the (verified-merged / confirmed) branches non-
    // interactively — mirroring the archive path's defaulted deletion. Branches
    // must be deleted after their worktrees are gone, so do it here.
    const taskDir = path.join(projectDir, 'tasks', taskId);
    const removed = await teardownTask(taskDir, taskId, { askBranchDeletion: false });
    if (!opts.keepBranches) {
      ...
      for (const { baseRepo, branch } of removed) {
        ...
        seenBr.add(key);
        uniq.push({ baseRepo, branch });
      }
      for (const { baseRepo, branch } of uniq) {
        const r = git.deleteBranch(baseRepo, branch, true); // force: verified merged / confirmed
...
async function handleOpen(projectDir, projectName, taskId, mode) {
  ...
  // --optional repos are excluded by default: instead of prompting, record them
  // as excluded silently — on create and on resume (covering tasks that predate
  // the flag). Pull one in with: wksp task repo <id> <repo> worktree
  const optionalSkipped = allRepos.filter(r =>
    r.optional &&
  ...
  if (optionalSkipped.length) {
    for (const r of optionalSkipped) taskExcludedSet.add(r.folderName);
    writeTaskSets(taskDir, taskSharedSet, taskExcludedSet);
  }
...
// Archived tasks, for the delete picker/fallback: [{ id, worktrees: 0, lastActive, archived: true }].
// Ordered by manifest archive time (newest first), falling back to dir mtime.
function listArchivedTasks(projectDir) {
  const ids = archive.listArchivedTaskIds(projectDir);
...
async function resolveTaskId(projectDir, sub, provided) {
  const liveTasks = listLiveTasks(projectDir);
  // delete is the only subcommand that can act on an archived task, so it's the
  // only one that reaches into archived-tasks/ for partial matches and the picker.
  const archivedTasks = sub === 'delete' ? listArchivedTasks(projectDir) : [];

  if (provided) {
    if (liveTasks.some(t => t.id === provided)) return provided;
    const liveMatches = liveTasks.filter(t => t.id.toLowerCase().includes(provided.toLowerCase()));
    if (liveMatches.length === 1) { console.log(`  → ${liveMatches[0].id}`); return liveMatches[0].id; }
    if (liveMatches.length > 1)   return pickTask(liveMatches, sub);

    // No live match. For delete, an exact archived id routes straight through
    // (handleDel already deletes it); otherwise try archived partial matches.
    if (sub === 'delete') {
      ...
      if (arcMatches.length > 1)   return pickTask(arcMatches, sub);
    }
    return provided; // no match — let the handler's own not-found logic run
  }

  const pickable = [...liveTasks, ...archivedTasks];
  if (!pickable.length) {
...
`);
  ...
  switch (sub) {
    case 'create':
    case 'resume':
      await handleOpen(projectDir, projectName, taskId, sub);
      break;
    case 'delete': {
...
module.exports = { run, resolveTaskId, listLiveTasks, handleOpen };
