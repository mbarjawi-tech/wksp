...
async function handleToShared(projectDir, taskId, repoArg) {
  if (!repoArg) { console.error('  Usage: wksp task share <id> <repo>'); process.exit(1); }
...
async function handleToWorktree(projectDir, taskId, repoArg) {
  if (!repoArg) { console.error('  Usage: wksp task worktree <id> <repo>'); process.exit(1); }
...
async function handleToExclude(projectDir, taskId, repoArg) {
  if (!repoArg) { console.error('  Usage: wksp task exclude <id> <repo>'); process.exit(1); }
...
async function handleLegacy(args) {
  ...
  if (flags.has('--del')) {
    ...
    warnDeprecated(`wksp task ${taskId} --to-shared ${posArgs[1] || '<repo>'}`, `wksp task share ${taskId} ${posArgs[1] || '<repo>'}`);
    await handleToShared(projectDir, taskId, posArgs[1]);
  } else if (flags.has('--to-worktree')) {
    warnDeprecated(`wksp task ${taskId} --to-worktree ${posArgs[1] || '<repo>'}`, `wksp task worktree ${taskId} ${posArgs[1] || '<repo>'}`);
    await handleToWorktree(projectDir, taskId, posArgs[1]);
  } else if (flags.has('--to-exclude')) {
    warnDeprecated(`wksp task ${taskId} --to-exclude ${posArgs[1] || '<repo>'}`, `wksp task exclude ${taskId} ${posArgs[1] || '<repo>'}`);
...
const SUBCOMMANDS = new Set(['create', 'resume', 'delete', 'rename', 'archive', 'unarchive', 'share', 'worktree', 'exclude']);
...
async function run(args) {
  ...
  Subcommands:
    ...
    share <id> <repo>              Switch a repo to shared path for this task only
    worktree <id> <repo>           Create or restore a worktree for a repo in this task
    exclude <id> <repo>            Exclude a repo from this task entirely
...
`);
  ...
  switch (sub) {
    ...
    case 'share':
      await handleToShared(projectDir, taskId, posArgs[2]); break;
    case 'worktree':
      await handleToWorktree(projectDir, taskId, posArgs[2]); break;
    case 'exclude':
      await handleToExclude(projectDir, taskId, posArgs[2]); break;
  }
}
