...
async function run() {
  ...
  for (const wt of wts) {
    if (wt.baseRepo) baseMap.set(normalizePath(wt.baseRepo), wt);
  }

  const projectCfg  = config.readProjectConfig(projectDir);
  const projectName = projectCfg.name || path.basename(projectDir);
  const W = 44;
  const nameW   = allRepos.length ? Math.max(...allRepos.map(r => path.basename(r.normalized).length)) + 2 : 20;
  const branchW = 26;
  ...
  for (const repo of allRepos) {
    ...
    const wt = baseMap.get(repo.normalized);
