...
`);
  ...
  for (const repo of allRepos) {
    ...
    if (!wt) {
      ...
      // A non-optional repo, however, was expected in this task: no worktree AND no
      // entry in task.json means nobody ever decided. It is silently absent from
      // everything a session is given, so say so rather than print an empty-looking row.
      unset.push(name);
