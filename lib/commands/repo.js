...
const { normalizePath, samePath, canonicalPath } = require('../paths');
...
async function handleRemove(projectDir, rawPath) {
  ...
  // Hoisted out of the task × worktree loop below: canonicalPath does a filesystem
  // round-trip and is deliberately not memoised (see lib/paths.js), so the one path
  // that is the same on every iteration is resolved once.
  const canonicalRepo = canonicalPath(normalized);
  ...
  if (fs.existsSync(tasksDir)) {
    for (const entry of fs.readdirSync(tasksDir, { withFileTypes: true })) {
      ...
      // `recover: true` — this command removes worktrees, so it earns putting a stranded
      // probe back rather than only reporting it.
      for (const wt of discoverWorktrees(taskDir, { recover: true })) {
        if (wt.strandedProbe) { stranded.push({ taskName: entry.name, wt }); continue; }
        // CANONICAL, and this one is load-bearing for safety rather than cosmetics.
        // `wt.baseRepo` is read out of the worktree's .git file, so it is git's long
        // on-disk form; `normalized` is whatever the user typed / repos.txt holds,
        // which on an 8.3 path is a different spelling of the same directory. When
        // the raw strings were compared, that mismatch dropped the worktree out of
        // `orphaned` — so the isCwdInside and probeRemovable guards below were never
        // reached for it, and `repo remove` deleted a worktree with a live shell
        // inside it while reporting nothing. A guard is only as good as the list it
        // is handed. (PLANNING #25.)
        if (!wt.baseRepo || !samePath(canonicalPath(wt.baseRepo), canonicalRepo)) continue;
        orphaned.push({ taskName: entry.name, taskDir, wt });
      }
    }
  }

  // Before the orphaned filter, because that filter cannot see a stranded probe: while a
  // worktree sits renamed aside its .git file is unreadable, so `baseRepo` is null and it
  // never matches this repo — it used to be skipped with no warning at all. The
  // deregistration below then ran anyway, and findWorktreeRegistration (which recovers a
  // wrecked worktree's branch name) walks repos.txt — so taking this repo out of repos.txt
  // while that probe is stranded makes the branch name PERMANENTLY unrecoverable. Refuse:
  // the whole point of this PR is that nothing silently skips and loses the trail.
  if (stranded.length) {
    console.error(`\n  Error: cannot remove — ${stranded.length} worktree probe(s) stranded by an earlier run`);
    console.error('  could not be moved back automatically:');
    for (const { taskName, wt } of stranded) {
      console.error(`     · ${taskName}/${wt.folderName} — currently at:\n         ${wt.strandedPath}`);
      console.error(`       Move it back by hand to:\n         ${wt.worktreeDir}`);
    }
    console.error('  A stranded worktree cannot say which repo it belongs to, so removing this one from');
    console.error('  repos.txt now could discard the only record of its branch. Nothing was touched.');
    console.error(`  Fix that, then re-run: wksp repo remove ${rawPath}\n`);
    process.exit(1);
  }
  ...
  const { removed } = repos.removeRepo(projectDir, rawPath);
  console.log(`  ✓  Removed from repos.txt: ${rawPath}`);
  // Only a pre-3.4.3 registry can hold two lines for one directory, and the teardown
  // above already covered both — so say that both lines went rather than let the line
  // above read as "one of them".
  if (removed > 1) console.log(`     (${removed} entries named that directory — all removed)`);
}
