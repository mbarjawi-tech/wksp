...
// the only case where deleting it can't lose anything the user typed. The trailing
// variants cover the one legitimate difference — the 4 → 5 / 5 → 6 append fallback
// (used when a restructured file has none of their anchor headings) writes
// `block.trimEnd() + '\n'` at the end of the file rather than the block's own trailing
// blank line. Anything else — a word changed, a line added, indentation touched — fails
// to match and the caller leaves the block alone.
function removeShippedBlock(content, block) {
  for (const variant of [block, block.trimEnd() + '\n', block.trimEnd()]) {
    if (content.includes(variant)) return content.replace(variant, '');
  }
  return null;
}
...
function migrate6to7(projectDir, dryRun, log = console.log) {
  ...
  if (!guidanceRelocated(content)) {
    ...
    content = anchor
      ? content.replace('\n' + anchor, '\n' + templates.HUB_POINTER_SECTION + anchor)
      : content.replace(/\s*$/, '') + '\n\n' + templates.HUB_POINTER_SECTION.trimEnd() + '\n';
