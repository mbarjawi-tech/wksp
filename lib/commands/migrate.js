...
`);
  ...
  if (unsafe) {
    ...
    // A project at a filesystem root resolves fine and is then refused forever, so the
    // refusal has to say what unsticks it rather than just what it will not do.
    console.error('  If a real project is there, move it into a subdirectory and re-run wksp migrate from inside it.\n');
    process.exit(1);
  }
