...
function migrate4to5(projectDir, dryRun, log = console.log) {
  ...
  // AGENTS.md is canonical (the 3 → 4 step above guarantees it); CLAUDE.md is
  // only the target for a project whose conversion couldn't complete.
  const target = fs.existsSync(agentsMd) ? agentsMd
...
function migrate5to6(projectDir, dryRun, log = console.log) {
  const agentsMd = path.join(projectDir, 'AGENTS.md');
  const claudeMd = path.join(projectDir, 'CLAUDE.md');
  // AGENTS.md is canonical (the 3 → 4 step guarantees it); CLAUDE.md is only the
  // target for a project whose conversion couldn't complete.
  const target = fs.existsSync(agentsMd) ? agentsMd
               : fs.existsSync(claudeMd) ? claudeMd
               : null;
...
module.exports = {
  run, applyMigrations,
  migrate0to1, migrate1to2, migrate2to3, migrate3to4, migrate4to5, migrate5to6,
  stripAlias,
};
