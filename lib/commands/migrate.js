...
//   - Add `.wksp-cache/` to the project's .gitignore (if not already listed)
// Returns { gitignoreChanged }.
function migrate2to3(projectDir, dryRun) {
  const gitignorePath = path.join(projectDir, '.gitignore');
  const entry = '.wksp-cache/';
  ...
  // Check if already present (any line that is exactly the entry, trimmed)
  const lines = raw.split('\n');
  const alreadyPresent = lines.some(l => l.trim() === entry);
  if (alreadyPresent) {
    console.log('  .gitignore — .wksp-cache/ already listed, nothing to add.');
    return { gitignoreChanged: false };
  }

  if (!dryRun) {
    // Append, ensuring a trailing newline before the new entry
  ...
  console.log('  .gitignore — added .wksp-cache/ (shared dep cache directory).');
  return { gitignoreChanged: true };
}
...
async function run(args) {
  ...
  What it does:
    ...
    Schema 2 → 3  Adds .wksp-cache/ to .gitignore (needed for shared dep directories).
`);
    process.exit(0);
  }
  ...
  // ── 2 → 3 ──────────────────────────────────────────────────────────────────
  if (from < 3) {
    migrate2to3(projectDir, dryRun);
    if (!dryRun) {
      config.setProjectConfig(projectDir, 'schemaVersion', 3);
      console.log('  ✓  .wksp: schemaVersion set to 3');
    }
    from = 3;
  }
...
module.exports = { run, migrate0to1, migrate1to2, migrate2to3, stripAlias };
