  ...
  if (showLive) {
    for (const id of liveNames) {
      ...
      tasks.push({
        ...
        worktrees: worktrees.map(w => ({ name: w.folderName, branch: w.currentBranch || null })),
      });
    }
  }
  if (showArchived) {
...
async function run(args) {
  ...
  if (flags.has('--all')) {
    ...
    for (const name of liveNames) {
      ...
      console.log(`  ${name.padEnd(nameW)}${'live'.padEnd(12)}${wts.length} worktree${wts.length === 1 ? '' : 's'}`);
    }
