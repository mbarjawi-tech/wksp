...
function worktreeCount(wts) {
  ...
  const n = `${live.length} worktree${live.length === 1 ? '' : 's'}`;
  return stranded.length ? `${n}, ${stranded.length} renamed aside` : n;
}
...
async function run(args) {
  ...
  if (!projectDir) {
    ...
    console.error('  Error: not inside a wksp project'); process.exit(1);
  }
  ...
  if (showLive) {
    if (!liveNames.length) {
      ...
      for (const name of liveNames) {
        const wts = discoverWorktrees(path.join(projectDir, 'tasks', name));
        const { live, stranded } = splitWorktrees(wts);
        console.log(`  ${name.padEnd(32)} ${live.length}${stranded.length ? ` (+${stranded.length} renamed aside)` : ''}`);
      }
    }
