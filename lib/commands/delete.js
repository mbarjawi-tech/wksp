...
async function run() {
  ...
  if (fs.existsSync(tasksDir)) {
    for (const entry of fs.readdirSync(tasksDir, { withFileTypes: true })) {
      ...
      // `recover: true` — deleting the whole project is about as destructive as intent gets.
      taskInfos.push({ name: entry.name, taskDir, wts: discoverWorktrees(taskDir, { recover: true }) });
    }
  }
  ...
  for (let i = 0; i < taskInfos.length; i++) {
    ...
    const cwdBlocked = refuseTaskCwd(task.taskDir);
    let ok = !cwdBlocked;

    // Nothing of this task's is touched once the cwd already disqualifies it.
    for (const wt of ok ? task.wts : []) {
      ...
      // `git worktree remove` has already deleted its contents.
