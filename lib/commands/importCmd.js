...
const { normalizePath, toPosix, samePathCanonical, unsafeProjectDirReason } = require('../paths');
...
async function executeImport(projectDir, projectName, bundle, resolvedRepos, mode) {
  ...
  for (const r of resolvedRepos) {
    ...
    if (!r.alreadyInProject) {
      ...
      // Canonical, and it has to stay in step with addRepo's own duplicate check:
      // addRepo THROWS on a duplicate, so a pre-check that misses one it would catch
      // turns this silent skip into a crash mid-import. Which means asking the same
      // question of the same values — addRepo compares normalizePath(rawPath), so this
      // normalizes too rather than relying on `localPath` already being in that form.
      // It is today (every localPath is normalizePath-derived), but a `/c/dev/x` value
      // — the spelling `wksp repo add`'s own help advertises — would put the two checks
      // back out of step, silently, which is the whole failure this pre-check exists for.
      if (!currentRepos.some(e => samePathCanonical(e.normalized, normalizePath(r.localPath)))) {
...
async function importIntoExistingProject(bundle) {
  ...
  if (projectDir) {
    ...
    if (!config.isProjectMarker(path.join(projectDir, config.PROJECT_MARKER))) {
      console.error(`  Error: "${projectDir}" is not a wksp project — its ${config.PROJECT_MARKER} is not a project marker`);
      console.error('         (a project marker carries a non-empty "name"), or there is none there at all.');
      process.exit(1);
    }
    const { name } = config.readProjectConfig(projectDir);
    projectName = name || path.basename(projectDir);
