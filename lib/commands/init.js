...
const { projectAgentsMd, planningMd, orchestrationMd, writeInstructionFiles,
        AGENTS_FILE, CLAUDE_FILE, GUIDANCE_FILE } = require('../templates');
const { REPOS_HEADER } = require('../repos');
...
async function run(args) {
  ...
  // Checked before the "already exists" test below, so the message names the real
  // problem: the home directory and a filesystem root always exist, and "already exists"
  // would say nothing about the damage. Writing the project marker here would overwrite
  // the global config, since both files are called `.wksp`.
  const unsafe = unsafeProjectDirReason(projectDir);
  if (unsafe) {
    console.error(`\n  Error: refusing to create a project at ${projectDir} — ${unsafe}.`);
    console.error('  Create the project in a subdirectory instead, e.g.:  wksp init my-project\n');
  ...
  fs.writeFileSync(path.join(projectDir, GUIDANCE_FILE), orchestrationMd(name));
  console.log(`  ✓  ${GUIDANCE_FILE}  (hub-only: delegation, PR review, stacked PRs, settings)`);
