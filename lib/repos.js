...
function readRepos(projectDir) {
  ...
  return fs.readFileSync(filePath, 'utf8')
    ...
    .map(l => {
      ...
      const raw        = l.replace(/ +--shared\b/, '').trim();
      const normalized = normalizePath(raw);
      const folderName = path.basename(normalized);
      return { raw, normalized, shared, folderName };
    });
}
...
function addRepo(projectDir, rawPath, shared) {
  ...
  entries.push({ raw: rawPath, normalized, shared: !!shared, folderName: path.basename(normalized) });
