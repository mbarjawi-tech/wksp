...
function reportProbes(taskDir, recovered, stranded, recover) {
  ...
  for (const sp of stranded) {
    ...
    console.warn(`     The next teardown of ${taskId} puts it back: wksp task archive ${taskId} (or delete / finish).`);
  }
}
...
//
// `recover: true` additionally puts it back, and belongs only to callers that already
// intend to move or delete the worktree; a read-oriented command must not mutate the
// filesystem just by looking. scanStrandedProbes explains why that split matters.
