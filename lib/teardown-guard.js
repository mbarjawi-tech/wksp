...
const { isInsideEitherForm } = require('./paths');
...
//
// A junction, an 8.3 short name or a symlinked temp dir can make one directory look
// like two different paths, so the LITERAL and the CANONICAL forms are both
// compared and either match refuses — see isInsideEitherForm in lib/paths.js for
// why a guard has to be eager in both directions.
//
// This used to do its own resolving with fs.realpathSync, which does not expand 8.3
// short names (only fs.realpathSync.native does), so the short-name half of that
// promise silently did nothing and the guard could fail OPEN — PLANNING #25.
function isCwdInside(dir) {
  const cwd = currentCwd();
  if (!cwd) return false;
  return isInsideEitherForm(cwd, dir);
}
