...
- **`wksp repo remove` could delete a worktree while your shell was sitting inside it — the 3.4.1 guard that exists to stop exactly that was never consulted.** wksp compared paths as strings and never canonicalised them, and on Windows one directory routinely has two names: any path component longer than 8 characters (or containing a space) also gets an 8.3 short name, so `C:\Users\runneradmin\…` and `C:\Users\RUNNER~1\…` are the same folder spelled two ways. git always reports the long form; wksp holds whichever form it was handed. Every "is this the same directory?" in the codebase was therefore a comparison between two spellings of one answer, and string equality got it wrong.

  The consequence that matters is not the mismatched strings, it is what the mismatch did to a **safety guard**. `wksp repo remove` matches each discovered worktree against the repo being removed *before* it runs any guard over it: the worktree's base repo is read out of its `.git` file (git's long spelling) and compared with the path in `repos.txt` (whatever spelling you registered). When those differ, the worktree silently dropped out of the command's own to-do list — so the cwd check and the removability probe never saw it, and `repo remove` went ahead and deleted a worktree with a live shell inside it, reporting nothing. A guard is only as good as the list it is handed, and this one was handed an empty list. It failed **open**, which is worse than failing loudly: the 3.4.1 release notes promised that junctions and 8.3 short names were covered by "comparing resolved forms too", and they were not — that comparison used `fs.realpathSync`, the JS implementation, which hands an 8.3 path straight back unchanged. Only `fs.realpathSync.native` expands short names, so the short-name half of that promise had never done anything.

  The same wrong comparison broke several non-destructive things in the same way, all of them the "which directory is this?" question asked of git:
  - **A worktree wrecked mid-teardown lost its branch name.** The recovery reads the base repo's worktree registry — the last place the branch survives after the `.git` file is gone — and that lookup mismatched, so a degraded `delete` / `archive` / `finish` reported `local branch NOT deleted: name unrecoverable (no .git file, no registration)` and `base repo unknown` instead of naming the branch and the repo. The whole point of that reporting (3.4.1) is that nothing is skipped in silence, and it was skipping the useful half.
  - **`wksp task unarchive` skipped a repo instead of marking it shared.** When the archived branch is already checked out in the base repo itself, the plan is meant to reuse the base repo in place; the check compared the path git reported against the registered one and concluded the branch was checked out in some *other* worktree, so the repo was dropped from the plan. (That comparison was also case-sensitive, which is wrong on a case-insensitive filesystem regardless of short names.)
  - **A repo registered under one spelling could not be removed by the other.** `wksp repo remove` threw `Repo not found in repos.txt` — *after* it had already torn down the orphaned worktrees — and the duplicate check in `repo add` would happily register one directory twice.

  The fix canonicalises **comparisons**, not stored values. A new `canonicalPath` resolves a path to its real on-disk name using `fs.realpathSync.native` (which expands 8.3 short names as well as following junctions and symlinks), falling back to `fs.realpathSync` and then to the plain resolved path. It never throws: paths that do not exist yet are the normal case in wksp — the worktree directory it is about to create, the archive folder it is about to rename into — so an unresolvable path canonicalises through its deepest existing ancestor with the remaining tail re-attached. `normalizePath` is deliberately unchanged, because its output is *written* to `repos.txt` and shown in prompts; canonicalising there would rewrite your registry, resolve junctions you deliberately registered, and still not fix anything, since a comparison is only correct when **both** sides are canonical.

  Identity lookups and safety guards now use **different** comparisons, on purpose:
  - **Lookups compare canonical forms only.** "Is the directory git just told me about the same one I am holding?" has exactly one right answer, and the two strings are two spellings of it.
  - **Guards match if EITHER the literal or the canonical form matches.** A guard's failure modes are not symmetric — matching when it shouldn't costs you a refusal you work around by `cd`-ing elsewhere; failing to match costs you a deleted worktree. Keeping the literal comparison is not redundancy: a worktree directory that is a junction pointing *outside* the task folder is, by canonical name, nowhere near what is about to be deleted, while by the only name you and `git worktree remove` will ever use, it is squarely inside it. Comparing canonical forms alone would make the guard miss that. Collapsing the two into one comparison reopens one of the two holes whichever way it is collapsed.

  The home-directory protection added in 3.4.2 was quietly exposed to the same trick and is now an either-form comparison too: the global config is `~/.wksp` and a project marker is `<project>/.wksp`, and the check that tells them apart compared `os.homedir()` against a path built by walking up from your shell's cwd. On any machine whose username is over 8 characters — which is most of them — those can be two different strings for one file, and a mismatch would have let the global config through as a project marker and put `wksp delete` back in front of your home directory.

  `canonicalPath` is deliberately **not** memoised, which is worth stating because caching it looks free and is not: Windows *recycles* 8.3 aliases, so once the directory owning `WKSP-P~1` is deleted the next similarly-named one is handed the same alias, and a cached entry stops being merely stale and starts naming a different directory — the exact class of wrong answer this change exists to remove. Validating a cache entry costs the same syscall as resolving it, so a validated cache saves nothing. Where a loop compares many paths against one fixed path, the caller resolves that side once outside the loop instead.

  Regression coverage builds an 8.3 scenario deliberately rather than waiting for a runner to supply one, and skips gracefully where the volume will not produce a short name (8.3 generation is a per-volume NTFS setting that is often switched off, and does not exist off Windows) — plus unit tests that drive the comparison helpers with an explicit short/long pair, so the behaviour stays pinned even where the filesystem will not cooperate. CI's Windows jobs no longer redirect `TEMP` away from the runner's 8.3 short path: that workaround was added alongside CI itself to get the suite green, and removing it is what keeps this fix honest.
...
- **wksp no longer mistakes your home directory for a project — `wksp delete` could offer to delete `~`.** The global config is `~/.wksp` and a project marker is `<project>/.wksp`: **the same filename**. Project resolution walked up from the current directory testing only whether a file called `.wksp` existed, so from anywhere under your home directory with no real project in between, it stopped at the **home directory itself** and handed it back as "the project". Twelve command modules resolve a project that way — every command except `init` — plus the schema check in `bin/wksp.js`, and the consequences ran from noisy to destructive:
  ...
  Making the marker's *content* load-bearing has consequences of its own, handled here rather than left for someone to hit:
  - **`name` is protected in project scope.** `wksp config clear name`, and `wksp config set name` to anything that is not a non-empty string (the value runs through `JSON.parse`, so `42` and `""` arrive as a number and an empty string), are refused. Clearing it made the project stop resolving — which took `wksp migrate --repair` out of reach along with it — and the obvious repair, `wksp config set name <x>` from outside a project, fell back to writing `name` into `~/.wksp`, making the global config project-shaped in turn; that fallback refuses too now. Renaming a project still works, and `--global` is deliberately untouched: `wksp config clear name --global` is the repair if a global `name` ever does get there.
  - **A UTF-8 BOM no longer hides a project.** PowerShell's `>` and `Out-File` write one by default, so a hand-edited `.wksp` picks one up easily — and `JSON.parse` throws on it, which now meant the project silently ceased to exist, with no visible cause. A leading BOM is stripped before parsing, in the marker check and in every project-config read.
  - **`not inside a wksp project` now says when a `.wksp` is the reason.** A rejected candidate is skipped in silence, which made "there is a `.wksp` here but it is not a project marker" indistinguishable from "there is no `.wksp` anywhere" — opposite problems wearing the same message, and the reason a cleared `name` or a BOM was undiagnosable from the CLI. That error now carries one extra line naming the first candidate rejected for its **shape**: `A .wksp exists at <path> but is not a project marker (a project marker carries a non-empty "name")`. The global config is never named: `~/.wksp` is skipped on every run from anywhere under your home directory, and that is business as usual, not a diagnosis.
  - **The refusals say how to get unstuck.** A genuine project *at* a filesystem root resolves normally and is then refused forever, so `migrate` and `delete` both name the way out — move it into a subdirectory and re-run there — instead of `delete`'s previous advice to remove the `.wksp` by hand, which for a real project would have orphaned it rather than let you delete it.
