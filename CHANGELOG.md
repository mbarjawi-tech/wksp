...
- **`wksp task finish` / `archive` / `delete` no longer tear a task half-down when something is sitting in its worktree.** Hit for real on Windows: the merge check passed cleanly, then `git worktree remove` deleted the worktree's **contents first — including its `.git` file** — and failed on the locked directory, because the invoking session's cwd was inside `tasks/<id>/worktrees/<repo>` and wksp, a child process, **inherits that cwd**. The retry was the dangerous half: with `.git` gone, the archive recorded the repo as `corrupted at archive` with a null branch, and every step that keys off that branch — deleting the local branch, fast-forwarding the base repo — was skipped **in silence**, while the run still printed its usual success lines. The folder rename then failed on the same lock, leaving a live task carrying a stale `archived.json` and an empty, locked `worktrees/<repo>`. Four changes, in the order they now run:
  - **Refuse up front.** Before removing a worktree, deleting a branch or renaming anything, teardown checks whether the process cwd is inside the task folder (`delete`, `archive`, `finish`, and the archived-task delete) and stops with `Cannot tear down <id> — your shell is inside <path>` plus the `cd` that fixes it, exit 1. The inherited cwd is a proxy for the launching shell's, and a parent shell's cwd keeps the directory locked no matter where wksp moves itself — so refusing is the only safe answer, and it happens before the plan, the prompt, the fetches and the fast-forwards. The containment test is `path.relative`-based rather than a string prefix: case-insensitive on Windows, and `…/tasks/foo-bar` is correctly **outside** `…/tasks/foo`, so a shell in one task never blocks the teardown of a similarly-named sibling. Junctions and 8.3 short names are covered by comparing resolved forms too
  - **Fail before destroying, not after.** Each worktree is probed for removability before the first destructive step — a rename to a temporary sibling name and straight back, since a directory the OS lets us rename is one it will let us delete, and a failed rename changes nothing. A lock held by *another* process (an editor, a dev server, a second shell) is therefore caught while the checkout is still whole: `Cannot tear down <id> — "<repo>" is locked (EBUSY). Nothing was touched`. In the vanishingly unlikely case the probe cannot put the folder back, it names where the folder went instead of leaving it to be discovered
  - **Never silently skip.** A repo whose worktree can't be read now reports every step that did not run for it — the worktree removal, the branch deletion, and (under `finish`) the base-repo fast-forward — with the `git -C … worktree prune` / `branch -D` / `pull --ff-only` commands that finish the job by hand, and the branch name recovered from the base repo's own worktree registry (the last place it survives, since git keeps listing a worktree until `worktree prune`). That name is deliberately kept out of the manifest's `branch` field: it is for reporting, and nothing verified it merged. `archive` prints `⚠ Archived <id> — with unfinished teardown.` instead of its plain `✓`, `finish` adds `⚠ <id> is NOT fully finished`, and both exit non-zero. `finish` also stops claiming `✓ All branches merged` when a worktree could not be read: those branches were never checked, and it now says so and counts only what it verified
  - **Told where it matters.** The generated task `AGENTS.md` already said to *suggest* `wksp task finish <id>` rather than run it; it now also says why it has to run from outside the task folder. Template text only — existing task files are not migrated — and `docs/reference.md` documents the whole refusal, the lock probe and the recovery path
  - **Get out of the way, then explain what's left.** Mirroring what `wksp delete` already did for a project folder, teardown chdirs the process out of the task folder before deleting or renaming it, degrading with an explanation rather than an `EBUSY` when it can't. A folder rename that still fails leaves the manifest in place on purpose: re-running `archive` recognises the half-state, prints `Recovering an interrupted archive of <id>`, and **re-uses that manifest** rather than capturing a new one — capturing again once the worktrees are gone would write a record with no branch and no tip sha, quietly costing the task its restorability. `delete` names the stale `archived.json` in its plan, so discarding the half-state is deliberate. Two smaller record fixes ride along: a branch an interrupted run had already deleted is recorded as gone rather than `branchKeptInBaseRepo`, and a task folder that can't be deleted says which command to re-run instead of raising a bare filesystem error
